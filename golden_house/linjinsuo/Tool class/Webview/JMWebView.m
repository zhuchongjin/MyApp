...
@interface JMWebView ()<UIWebViewDelegate,JMWebViewProgressDelegate ,WKUIDelegate,WKNavigationDelegate,WKScriptMessageHandler>
@property (nonatomic, strong) JSContext *context;
@property (nonatomic, strong) JMWebViewProress *progressProxy;
@property (nonatomic, strong) NSProgress *estimatedProgress;
#else
@interface JMWebView ()<JMWebViewProgressDelegate ,UIWebViewDelegate,JMWebViewProgressDelegate>
@property (nonatomic, strong) JSContext *context;
#endif
...
- (void)loadUrl:(NSString *)url{
    ...
    NSAssert(url.length, @"Error JMWebView loadURL: is not allow nil or empty");
...
- (void)webView:(WKWebView *)webView decidePolicyForNavigationAction:(WKNavigationAction *)navigationAction decisionHandler:(void (^)(WKNavigationActionPolicy))decisionHandler{
    ...
    if (!navigationAction.targetFrame) {
        ...
        decision = [self.delegate webView:self shouldStartLoadWithRequest:navigationAction.request navigationType:(JMWebViewNavigationType)navigationAction.navigationType];
    }
    if (!decision) {
        decisionHandler(WKNavigationActionPolicyCancel);
        return;
...
- (BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType{
    if ([self.delegate respondsToSelector:@selector(webView:shouldStartLoadWithRequest:navigationType:)]) {
           return  [self.delegate webView:self shouldStartLoadWithRequest:request navigationType:(JMWebViewNavigationType)navigationType];
    }else{
        return YES;
    }
...
#pragma mark JMWebViewProgress Delegate
...
//    [_bridge registerHandler:@"userid" handler:^(id data, WVJBResponseCallback responseCallback) {
//        NSLog(@"userid called: %@", data);
//        
//        
//    }];
    ...
    [_bridge callHandler:@"jsCallBacks" data:@{ @"lat":@"132.34342" }];
    
    [_bridge registerHandler:@"getLocation" handler:^(id data, WVJBResponseCallback responseCallback) {
        NSLog(@"getLocation called: %@", data);
        responseCallback(@"{lat:132.897378}");
    ...
    [_bridge registerHandler:@"checkApp" handler:^(id data, WVJBResponseCallback responseCallback) {
        NSLog(@"checkApp called: %@", data);
        responseCallback(@"linyi");
        
    }];
    
    [_bridge registerHandler:@"articleLink" handler:^(id data, WVJBResponseCallback responseCallback) {
        NSLog(@"articleLink called: %@", data);
...
- (void)initializeUIWebView:(UIWebView *)webView{
    _progressProxy = [[JMWebViewProress alloc]init];
...
- (void)registerNativeHelperJS{
    ...
    if (file) {
        ...
        NSURL *url = [[JMWebView bundleForName:@"JMWebView"] URLForResource:@"nativehelper" withExtension:@"js"];
