...
{
    [self handleLoading];
    CGFloat hudW = [text boundingRectWithSize:CGSizeMake(MAXFLOAT, 15) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:14]} context:nil].size.width;
...
-(void)showLoading
{
    ...
    HXHUD *hud = [[HXHUD alloc] initWithFrame:CGRectMake(0, 0, 80, 80) imageName:@"" text:@""];
    [hud showloading1];
    hud.alpha = 0;
    hud.tag = 10086;
    [self addSubview:hud];
...
- (void)showloading1
{
    UIActivityIndicatorView *loading = [[UIActivityIndicatorView alloc] initWithActivityIndicatorStyle:UIActivityIndicatorViewStyleWhiteLarge];
    [loading startAnimating];
    loading.center = CGPointMake(self.centerX,self.centerY);
    [self addSubview:loading];
//    loading.frame = self.imageView.frame;
}
